...
fn find_closest_intersection(wire1 : &str, wire2 : &str) -> (i32, i32) {
    // for each match/collision calculate a sum of distances for each wire
    // get a min of distances
    let w1 : Vec<&str> = wire1.split(",").collect();
    let w2 : Vec<&str> = wire2.split(",").collect();

    let mut matches = HashSet::new();
    let mut distance1 = HashMap::new();
    let mut distance2 = HashMap::new();
    ...
    for p in w1 {
        ...
        if x_mod == 1 {
            for i in x+1..(x+offset+1) {
                distance1.insert((i, y), dist);
                dist += 1;
            }
        } else if x_mod == -1 {
            for i in ((x-offset)..x).rev() {
                distance1.insert((i, y), dist);
                dist += 1;
            }
        }
        ...
        if y_mod == 1 {
            for i in y+1..(y+offset+1) {
                distance1.insert((x, i), dist);
                dist += 1;
            }
        } else if y_mod == -1 {
            for i in ((y-offset)..y).rev() {
                distance1.insert((x, i), dist);
                dist += 1;
            }
        }
    ...
    for p in w2 {
        ...
        if x_mod == 1 {
            for i in x+1..(x+offset+1) {
                if distance1.contains_key(&(i, y)) {
                    matches.insert((i, y));
                }
                distance2.insert((i, y), dist);
                dist += 1;
            }
        } else if x_mod == -1 {
            for i in ((x-offset)..x).rev() {
        ...
        if y_mod == 1 {
            for i in y+1..(y+offset+1) {
                if distance1.contains_key(&(x, i)) {
                    matches.insert((x, i));
                }
                distance2.insert((x, i), dist);
                dist += 1;
            }
        } else if y_mod == -1 {
            for i in ((y-offset)..y).rev() {
    ...
    let mut manhattan_distances = matches.iter()
                                         .map(|(px, py)| px.abs() + py.abs())
                                         .collect::<Vec<i32>>();
    manhattan_distances.sort();

    let mut wire_distances = matches.iter()
                                    .map(|(px, py)| distance1.get(&(*px, *py)).unwrap() + distance2.get(&(*px, *py)).unwrap())
                                    .collect::<Vec<i32>>();
    wire_distances.sort();
    
    return (manhattan_distances[0], wire_distances[0]);
}
...
fn main() {
    ...
    let (manhattan, wire) = find_closest_intersection(wire1, wire2);
    println!("Distances {} {}", manhattan, wire);
}

#[test]
fn test() {
    assert_eq!((6, 30), find_closest_intersection("R8,U5,L5,D3", "U7,R6,D4,L4"));
    assert_eq!((159, 610), find_closest_intersection("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"));
    assert_eq!((135, 410), find_closest_intersection("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"));
}